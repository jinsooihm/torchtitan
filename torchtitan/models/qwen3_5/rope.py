...
def _compute_mrope_cache_from_raw(
    config: "MRoPE.Config",
    ...
    pos = position_ids.to_local() if isinstance(position_ids, DTensor) else position_ids
    ...
    # ``pos`` is (num_tokens, 3); the last axis selects temporal/height/width.
    t_pos = pos[..., 0].long()
    ...
    for dim, offset in enumerate((1, 2), start=1):
        length = config.mrope_section[dim] * 3
...
class MRoPE(CosSinRoPE):
            ...
            *,
            ...
            if positions is not None and positions.ndim == 2:
                ...
                if positions.shape[0] != num_tokens:
                    raise ValueError(
                        f"MRoPE positions length ({positions.shape[0]}) must "
                        f"match num_tokens ({num_tokens})."
                    )
                return _compute_mrope_cache_from_raw(self, rope_cache, positions)
            return CosSinRoPE.Config.prepare_cache(
                self,
                rope_cache,
                positions=positions,
    ...
    def _compute_mrope_cache(self, position_ids: torch.Tensor) -> torch.Tensor:
        ...
        return _compute_mrope_cache_from_raw(cfg, self.cache, position_ids)
